module.exports = app => {
    ...
    router.get('/news/init', async (req, res) => {
        ...
        const newsTitles = ["UI改造日志第四期：背包系统优化在即，局内快捷消息更智能！", "《五虎上将交响曲》揭秘，一起来看看你的音乐公开课随堂笔记吧！", "0元免流畅玩包，轻轻松松上王者", "王者荣耀联合乘车码送豪华大礼，五五开黑不氪金！", "五虎有灵，音乐发声，王者荣耀音乐公开课来啦！", "未成年人防沉迷新规接入公告", "5月15日全服不停机优化公告", "5月13日“演员”惩罚名单", "5月13日外挂专项打击公告", "5月13日净化游戏环境声明及处罚公告", "峡谷迎初夏，好礼领不停", "黄忠-烈魂五虎上将限定皮肤即将上架，缤纷好礼齐降临", "“五五打卡游”活动开启", "与我为伍，彼此守护，活动专属皮肤&amp;五五局内表现获取全攻略", "五五开黑节，赵云-龙胆即将上架，多重福利来袭", "2020年KPL春季赛季后赛赛程赛制公布，5月28日16:00热血开战", "【原创内容大赛音乐比赛】优秀作品合集（二）", "大众赛事合作赛道全面开启，携手合作伙伴共建王者电竞生态", "【KPL今日预报】南京Hero久竞 vs DYG，DYG.久诚迎战老东家能否取胜", "关于责成TS俱乐部就欠薪问题进行说明的通知"]
    ...
    router.get('/heroes/list', async (req, res) => {
        ...
        const cats = await Category.aggregate([
            { $match: { parent: parent._id } },
            { $lookup: { from: 'heroes', localField: '_id', foreignField: 'categories', as: 'heroList'} }
        ])
        const subCats = cats.map( v => v._id )
        cats.unshift({
            name: '热门',
    ...
    router.get('/articles/:id', async (req, res) => {
        ...
        data.related = await Article.find().limit(2).where({
